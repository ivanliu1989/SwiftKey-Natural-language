...
wordcloud(names(freq_onetoken), freq_onetoken, min.freq=30, colors=brewer.pal(6, 'Dark2'), rot.per=.2)
wordcloud(names(freq_bitoken), freq_bitoken, min.freq=12, colors=brewer.pal(6, 'Dark2'), rot.per=.2)
wordcloud(names(freq_tritoken), freq_tritoken, min.freq=3, colors=brewer.pal(6, 'Dark2'), rot.per=.2)
wordcloud(names(freq_quatrtoken), freq_quatrtoken, min.freq=2, colors=brewer.pal(6, 'Dark2'), rot.per=.2)
```
...
ggplot(wf_alltoken,aes(x=word,y=freq)) + 
    ...
    xlab('Terms')+ylab('Frequency') + theme(legend.position='none', axis.title.x = element_text(size=12), 
...
```{r echo=FALSE, warning=FALSE, fig.width=8, fig.height=2}
# setwd('/Users/ivan/Work_directory/SwiftKey')
